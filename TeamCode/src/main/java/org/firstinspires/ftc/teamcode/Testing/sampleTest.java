...
import static org.firstinspires.ftc.teamcode.TeleOp.meet2.FiniteState.AUTO_INTAKE_GROUND_END;
...
public class sampleTest extends OpMode {
    ...
    private Pose scoreBucketPose = new Pose(130, 27, Math.toRadians(135));

    private Pose groundIntakePose1 = new Pose(117, 35, Math.toRadians(181));
    private Pose groundIntakePose2 = new Pose(117, 25, Math.toRadians(180));
    private Pose groundIntakePose3 = new Pose(117, 30, Math.toRadians(220));


//private Pose wallIntakeAdjustPose = new Pose(130.5, 116, Math.toRadians(0));

    private Pose parkPose = new Pose(75, 58, Math.toRadians(270));
    ...
    public void buildPaths() {
        ...
        groundToScore2 = new Path(new BezierLine(new Point(groundIntakePose2), new Point(scoreBucketPose)));

        groundIntake3 = new Path(new BezierLine(new Point(scoreBucketPose), new Point(groundIntakePose3)));
        groundToScore3 = new Path(new BezierLine(new Point(groundIntakePose3), new Point(scoreBucketPose)));

        parkPath = new Path(new BezierLine(new Point(scoreBucketPose), new Point(parkPose)));
    ...
    public void autonomousPathUpdate() {
        ...
        switch (finiteState) {
            ...
            case INTAKE_GROUND_START:
                if (clawTimer.milliseconds() > clawServoTime) {
                    ...
                    if (numberOfDelivery == 3) {
                        ...
                        groundIntake3.setLinearHeadingInterpolation(scoreBucketPose.getHeading(), groundIntakePose1.getHeading());
                    }
            ...
            case DELIVERY_HIGH_BUCKET_PIVOT:
                if ((arm.pivotMotor.getCurrentPosition() + 200 >= arm.maximumPivotBucket) && (!follower.isBusy())) {
