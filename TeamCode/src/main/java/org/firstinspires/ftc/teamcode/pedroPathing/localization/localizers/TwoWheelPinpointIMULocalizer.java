...
public class TwoWheelPinpointIMULocalizer extends Localizer {
    ...
    /*********
     *  currentVolecity is set as a Pose with (x, y, heading) in this Loczlizer.
     * **************/
    ...
    // Go Builda: 2000 cpr, 32mm diameter, TICKS per MM: 19.894, TICKS per inch: 0.7832 Comment By AZ
    //REV: 8192 * 1.37795 * 2 * Math.PI * 0.5008239963;
    public static double STRAFE_TICKS_TO_INCHES = 0.002;//REV 8192 * 1.37795 * 2 * Math.PI * 0.5018874659;
...
//        strafeEncoderPose = new Pose(-107.9/25.4+0.25, -1.1/25.4-0.23, Math.toRadians(90));
        ...
        // With respect to CENTER of the robot, VIEW FROM the top of the robot looking downwards with robot moving forward Comment by Yjin

//        forwardEncoderPose = new Pose(-60/25.4, -140/25.4, 0);
//        strafeEncoderPose = new Pose( -60/25.4, 140/25.4, Math.toRadians(90));

        forwardEncoderPose = new Pose(-2.36, -5.31, 0);
        strafeEncoderPose = new Pose( -2.36, 5.5, Math.toRadians(90));

        // Localization of encoder pose with respect to POD offset

       // forwardEncoderPose = new Pose(-135/25.4,  -55/25.4,0);
       // strafeEncoderPose = new Pose(  -137.5/25.4, 52/25.4, Math.toRadians(90));
        ...
        // connected to port 0 encoder for meet 2 robot
        forwardEncoder = new Encoder(hardwareMap.get(DcMotorEx.class, "rightRear"));
       //connected to port 1 encoder for meet 2 robot
        ...
        /*********************************************
        FOR LATERAL ZERO POWER TEST PAY ATTENTION TO THIS FORWARD OR REVERSE
        ************************************************/
